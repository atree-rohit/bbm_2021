	...
	methods:{
		...
		init () {
			...
			this.map_data.forEach(o => {
				if(Object.keys(this.state_data).indexOf(o.state) != -1){
					...
					unmatched_state_names.push(o)
				}
			});
			// console.table(unmatched_state_names)
		...
		mapPoints(){
			...
			if(this.selected != 'All'){
				...
				points = this.map_data
					.filter((o) => o.state == null)
					.map((o) => {
						let op = []
						if(o.location == undefined){
							op = [o.lon, o.lat,o.id, JSON.stringify(o)]
						} else {
							let coords = o.location.split(",")
							op = [coords[1], coords[0], o.id, JSON.stringify(o)]
						}
						return op
					})
			}
			...
			if(points.length > 0){
				...
				.data(points).enter()
					...
					.on('mouseover', function (d, i) {
							that.tooltip.html(
								`<table>
								<tr><td>State</td><td>${d[2]}</td></tr>
								<tr><td>Unique Taxa</td><td>${d[3]}</td></tr>
								</table>`)
								.style('visibility', 'visible');
							})
						.on('mousemove', function () {
